...
#   include Helpers::Model::Relations
#
...
#
### Mass-assignment
# u1 = User.new(name: 'Bob', address: { city: 'Vegas' })
# u1.eql?(u1.address.parent) # => true
#
### Explicit assignment
# u2 = User.new(name: 'Dan')
# u2.address = { city: 'LA' }
# u2.eql?(u2.address.parent) # => true
#
### Lazy assignment
# u3 = User.new(name: 'Luke')
# u3.eql?(u3.address.parent) # => true
#
  ...
  module Relations
    ...
    module ClassMethods
      ...
      def relation_attributes
        return @relation_attributes if @relation_attributes

        rel_attrs = attribute_set.select do |attribute|
      ...
      def relate(object, method_return)
        if object.kind_of?(Array)
          ...
          object.define_singleton_method(:parent) { method_return }
          object
        end
      end
    ...
    module InstanceMethods
      ...
      def dup
        if respond_to?(:parent)
          self.class.relate(dup_not_related, parent)
        else
          dup_not_related
        end
      end
